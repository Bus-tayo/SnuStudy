...
export default function MenteeBottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-[80] flex justify-center bg-white border-t border-border/60 pb-safe">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center gap-1 rounded-2xl px-5 py-2 transition-all duration-200 ${isActive
